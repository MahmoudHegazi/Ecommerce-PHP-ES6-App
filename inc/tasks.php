...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  if (isset($data['taskname']) && isset($data['duration']) && isset($data['start_date']) && isset($data['finish_date'])){
    ...
    $rc = $stmt->bind_param("siss", $task_name, $duration, $start_date, $finish_date);
    if ($rc===false){
      $res_data = new stdClass();
    ...
    $task_name = test_input('Python king1') . $i;
    $duration = $i . ' days';
    $start_date = '1994-12-15';
    $finish_date = '1994-12-15';


    // set parameters and execute
    /*
    $task_name = test_input($data['taskname']) . $i;
    $duration = test_input($data['duration']) . $i;
